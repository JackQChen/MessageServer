...
namespace MessageLib
{
    ...
    public class FileCache : IDisposable
    {
        ...
        /// </summary>
        public bool Empty { get { return _entriesByKey.Count == 0; } }

        /// <summary>
        /// Get the file cache size
        /// </summary>
        public int Size { get { return _entriesByKey.Count; } }
        ...
        public Tuple<bool, byte[]> Find(string key)
        {
            using (new ReadLock(_lockEx))
            {
                ...
                MemCacheEntry cacheValue;
                if (!_entriesByKey.TryGetValue(key, out cacheValue))
        ...
        private class FileCacheEntry
        {
            ...
            {
                var key = e.FullPath.Replace(entry._path, "");
                var file = e.FullPath;
            ...
            {
                var oldKey = e.OldFullPath.Replace(entry._path, "");
                var oldFile = e.OldFullPath;
                var newKey = e.FullPath.Replace(entry._path, "");
        ...
        private bool RemovePathInternal(string path)
        {
            using (new WriteLock(_lockEx))
            {
                ...
                FileCacheEntry cacheValue;
                if (!_pathsByKey.TryGetValue(path, out cacheValue))
                    return false;
