    ...
    class AsyncSocketServer : TcpSocketServerBase, IActiveConnector
    {
        ...
        public override bool Start()
        {
            try
            {
                ...
                {
                    AppServer.Logger.ErrorFormat("Failed to allocate buffer for async socket communication, may because there is no enough memory, please decrease maxConnectionNumber in configuration!", e);
                    return false;
                }
        ...
        private IAppSession ProcessNewClient(Socket client, SslProtocols security)
        {
            ...
            int sessionID = AppServer.CreateAppSessionID();

            if (security == SslProtocols.None)
                socketSession = new AsyncSocketSession(sessionID, client, socketEventArgsProxy);
            else
                socketSession = new AsyncStreamSocketSession(sessionID, client, security, socketEventArgsProxy);
        ...
        public override void ResetSessionSecurity(IAppSession session, SslProtocols security)
        {
            ...
            if (security == SslProtocols.None)
                socketSession = new AsyncSocketSession(sessionID, session.SocketSession.Client, socketAsyncProxy, true);
            else
                socketSession = new AsyncStreamSocketSession(sessionID, session.SocketSession.Client, security, socketAsyncProxy, true);
