...
public class Sdk
{
    ...
    static Sdk()
    {
        var path = AppDomain.CurrentDomain.BaseDirectory + HPSOCKET_DLL_PATH;
        if (!File.Exists(path))
        {
            if (Environment.Is64BitProcess)
                File.WriteAllBytes(path, Resources.SocketLibx64);
            else
                File.WriteAllBytes(path, Resources.SocketLibx86);
        }
    }
    ...
    /// HPSocket的文件路径
    /// </summary>
    public const string HPSOCKET_DLL_PATH = "SocketLib.dll";
