...
namespace MessageLib.SocketBase
{
        ...
        where TAppSession : AppSession<TAppSession, TRequestInfo>, IAppSession, new()
    {
        ...
        private ConcurrentDictionary<int, TAppSession> m_SessionDict = new ConcurrentDictionary<int, TAppSession>();
        ...
        /// <returns></returns>
        protected override bool RegisterSession(int sessionID, TAppSession appSession)
        {
        ...
        private void ClearIdleSession(object state)
        {
            if (Monitor.TryEnter(state))
            {
                ...
                {
                    Logger.ErrorFormat("Clear idle session error!", e);
                }
                finally
                {
                    Monitor.Exit(state);
        ...
        private KeyValuePair<int, TAppSession>[] SessionSource
        {
            get
            {
                if (Config.DisableSessionSnapshot)
        ...
        private KeyValuePair<int, TAppSession>[] m_SessionsSnapshot = new KeyValuePair<int, TAppSession>[0];
