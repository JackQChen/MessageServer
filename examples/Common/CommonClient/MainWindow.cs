...
using Gtk;
using MessageLib;
using MessageType = CommonService.MessageType;
...
namespace CommonClient
{
    ...
    {
        [UI] private Button button1, button2, button3, button4, button5, button6;
        [UI] private TextView txtLog = null;

        CommonClient client;
        Action<string> actlog;
        Action<int> actProgress;
        BlockingCollection<string> logList;

        string strLog;

        public MainWindow() : this(new Builder("MainWindow.glade"))
        {
            this.txtLog.AddTickCallback((w, f) =>
            {
                if (string.IsNullOrEmpty(strLog))
                    return true;
                ((TextView)w).Buffer.Text = strLog;
                return true;
            });
        }
        ...
        private MainWindow(Builder builder) : base(builder.GetRawOwnedObject("MainWindow"))
        {
            ...
            button1.Clicked += this.button1_Click;
            button2.Clicked += this.button2_Click;
            button3.Clicked += this.button3_Click;
            button4.Clicked += this.button4_Click;
            button5.Clicked += this.button5_Click;
            button6.Clicked += this.button6_Click;
        }
...
MD5={2}", msg.FileName, msg.FileSize.ToFileSize(), msg.FileMD5));
                        var dir = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ReceivedFiles");
                        if (!Directory.Exists(dir))
                            Directory.CreateDirectory(dir);
                        var filePath = System.IO.Path.Combine(dir, msg.FileName);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string filePath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Send", "MainWindow.glade");
