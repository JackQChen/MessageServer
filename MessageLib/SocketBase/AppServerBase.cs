...
using MessageLib.SocketBase.Security;
...
namespace MessageLib.SocketBase
{
        ...
        where TAppSession : AppSession<TAppSession, TRequestInfo>, IAppSession, new()
    {
        ...
        private long m_TotalHandledRequests = 0, m_TotalSent = 0, m_TotalReceived = 0;
        ...
        /// <summary>
        /// Gets the total received number.
        /// </summary>
        ...
        private void SetupBasic(IRootConfig rootConfig, IServerConfig config, ISocketServerFactory socketServerFactory)
        {
            ...
            if (socketServerFactory == null)
            {
                var socketServerFactoryType =
                    Type.GetType("MessageLib.SocketEngine.SocketServerFactory, MessageLib", true);
        ...
        private bool SetupFinal()
        {
            ...
            {
                Logger.ErrorFormat("Failed to create ServerSummary instance!", e);
                return false;
            }
        ...
        private bool SetupLogFactory(ILogFactory logFactory)
        {
            ...
            //ServiceLogFactory is default log factory
            if (LogFactory == null)
                LogFactory = new ServiceLogFactory();
            return true;
        }
        ...
        private bool SetupSecurity(IServerConfig config)
        {
            ...
            {
                Logger.ErrorFormat("Failed to initialize certificate!", e);
                return false;
            }
        ...
        public virtual bool Start()
        {
            ...
            {
                Logger.ErrorFormat("One exception wa thrown in the method 'OnStartup()'.", e);
            }
            finally
            {
                Logger.Info(string.Format("[{0}]已启动", Name));
            }
        ...
        public virtual void Stop()
        {
            ...
            Logger.Info(string.Format("[{0}]已停止", Name));
        }
        ...
        /// <summary>
        /// create a new AppSession ID
        /// </summary>
        /// <returns>the new created session ID</returns>
        int IAppServer.CreateAppSessionID()
        {
            Interlocked.Increment(ref m_sessionID);
            return m_sessionID;
        }
        ...
        /// search both in the appserver's root and in the root dir if the isolation level has been set other than 'None'.
        /// </summary>
        ...
        void IStatusInfoSource.UpdateServerStatus(string name, object value)
        {
            switch (name)
            {
                case StatusInfoKeys.TotalSent:
                    Interlocked.Add(ref this.m_TotalSent, Convert.ToInt64(value));
                    break;
                case StatusInfoKeys.TotalReceived:
                    Interlocked.Add(ref this.m_TotalReceived, Convert.ToInt64(value));
                    break;
                default:
                    throw new NotSupportedException("Unsupported Name:" + name);
            }
        }
        ...
        protected virtual void UpdateServerStatus(StatusInfoCollection serverStatus)
        {
            ...
            var totalSent0 = serverStatus.GetValue<long>(StatusInfoKeys.TotalSent, 0);
            var totalSent = this.TotalSent;
            serverStatus[StatusInfoKeys.SendingSpeed] = (totalSent - totalSent0) / now.Subtract(serverStatus.CollectedTime).TotalSeconds;
            serverStatus[StatusInfoKeys.TotalSent] = totalSent;

            var totalReceived0 = serverStatus.GetValue<long>(StatusInfoKeys.TotalReceived, 0);
            var totalReceived = this.TotalReceived;
            serverStatus[StatusInfoKeys.ReceivingSpeed] = (totalReceived - totalReceived0) / now.Subtract(serverStatus.CollectedTime).TotalSeconds;
            serverStatus[StatusInfoKeys.TotalReceived] = totalReceived;
