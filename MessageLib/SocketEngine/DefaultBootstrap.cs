...
namespace MessageLib.SocketEngine
{
    ...
    public partial class DefaultBootstrap : IBootstrap, ILoggerProvider, IDisposable
    {
        ...
        public DefaultBootstrap(IEnumerable<IWorkItem> appServers)
            : this(new RootConfig(), appServers, new ServiceLogFactory())
        {
        ...
        public DefaultBootstrap(IRootConfig rootConfig, IEnumerable<IWorkItem> appServers)
            : this(rootConfig, appServers, new ServiceLogFactory())
        {
        ...
        public DefaultBootstrap(IRootConfig rootConfig, IEnumerable<IWorkItem> appServers, ILogFactory logFactory)
        {
            ...
            if (!rootConfig.DisablePerformanceDataCollector)
            {
                ...
                m_GlobalLog.Debug("性能监控初始化完成");
            }
            m_GlobalLog.Debug("引导程序初始化完成");
            m_Initialized = true;
        }
        ...
        private IWorkItem InitializeAndSetupWorkItem(WorkItemFactoryInfo factoryInfo)
        {
            ...
            try
            {
                ...
                m_GlobalLog.DebugFormat("[{0}]已创建", factoryInfo.Config.Name);
            }
            catch (Exception e)
            {
                m_GlobalLog.ErrorFormat(string.Format("[{0}]创建失败", factoryInfo.Config.Name), e);
                return null;
            }
            ...
            try
            {
                ...
                m_GlobalLog.DebugFormat("[{0}]初始化完成", appServer.Name);
            }
            catch (Exception e)
            {
                m_GlobalLog.Error(e);
        ...
        public virtual bool Initialize(Func<IServerConfig, IServerConfig> serverConfigResolver, ILogFactory logFactory)
        {
            ...
            using (var factoryInfoLoader = GetWorkItemFactoryInfoLoader(m_Config, logFactory))
            {
                ...
                bootstrapLogFactory.ExportFactory.EnsureInstance();
            ...
            {
                m_GlobalLog.ErrorFormat("Failed to register remoting access service!", e);
                return false;
            }
        ...
        {
            m_GlobalLog.ErrorFormat(string.Format("The server {0} threw an exception.", ((IWorkItemBase)sender).Name), e.Exception);
        }

        void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            m_GlobalLog.FatalFormat("The process crashed for an unhandled exception!", (Exception)e.ExceptionObject);
        }
        ...
        public StartResult Start()
        {
            ...
            foreach (var server in m_AppServers)
            {
                if (!server.Start())
                {
                    m_GlobalLog.InfoFormat("[{0}]启动失败", server.Name);
                }
                else
                {
                    succeeded++;
                    ...
                    {
                        m_GlobalLog.InfoFormat("[{0}]已启动", server.Name);
                    }
                }
            }
        ...
        {
            var bootstrapIpcPort = string.Format("Socket.Bootstrap[{0}]", Math.Abs(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(System.IO.Path.DirectorySeparatorChar).GetHashCode()));
