...
namespace MessageLib
{
    ...
    public class HttpResponse
    {
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine(string.Format("Status phrase: {0}", StatusPhrase));
            sb.AppendLine(string.Format("Protocol: {0}", Protocol));
            sb.AppendLine(string.Format("Headers: {0}", Headers));
            for (int i = 0; i < Headers; i++)
            {
                var header = Header(i);
                sb.AppendLine(string.Format("{0} : {1}", header.Item1, header.Item2));
            }
            sb.AppendLine(string.Format("Body: {0}", BodyLength));
            sb.AppendLine(Body);
            return sb.ToString();
        }
        ...
        public HttpResponse SetCookie(string name, string value, int maxAge = 86400, string path = "", string domain = "", bool secure = true, bool httpOnly = false)
        {
            ...
            int valueSize = _cache.Size - valueIndex;
